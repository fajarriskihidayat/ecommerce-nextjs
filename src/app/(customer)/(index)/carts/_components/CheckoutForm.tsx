...
const SubmitButton = (props: { count: number }) => {
  const { pending } = useFormStatus();

  return (
    <button
      type="submit"
      className={`p-[12px_24px] bg-[#0D5CD7] rounded-full text-center font-semibold text-white hover:bg-opacity-80 disabled:bg-opacity-80 ${
        props.count === 0 && "bg-opacity-80"
      }`}
      disabled={pending || props.count === 0}
    >
      ...
      <div className="flex flex-1 flex-col shrink-0 gap-4 h-fit">
        ...
        <div className="w-full bg-white border border-[#E5E5E5] flex flex-col gap-[30px] p-[30px] rounded-3xl">
          ...
          <div className="flex flex-col gap-3">
            <SubmitButton count={products.length} />
            <a
              href=""
